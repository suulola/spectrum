      ...
      next: {
        "*[1-2]": () => {
          return new Promise((resolve) => {
            ...
            menu.session
              .get(cardList[menu.val - 1].split(".")[1])
              .then((val) => {
                bankDetail = userData.account_cards[menu.val - 1];
                authCode = "";
                authCode = val;
              });
    ...
    menu.state("chargeCardState", {
      run: () => {
        menu.session.get("fundCardAmt").then((val) => {
          fundAccount(val, authCode, baseUser, bankDetail).then(
            (data) => {
              ...
              if (data.status === true) {
                ...
                menu.end(
                  "Transaction was unsuccessful, Please try again later"
                );
              }
            },
            (err) => {
              menu.end("Transaction was unsuccessful, Please try again later");
